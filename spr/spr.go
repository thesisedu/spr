...
func (sd *stackediff) UpdatePullRequests(ctx context.Context, reviewers []string, count *uint) {
	...
	if commitsReordered(localCommits, githubInfo.PullRequests) {
		...
		for i := range githubInfo.PullRequests {
			go func(i int) {
				...
				sd.github.UpdatePullRequest(ctx, sd.gitcmd, githubInfo.PullRequests, pr, pr.Commit, nil, githubInfo)
				wg.Done()
			}(i)
		}
	...
	for i := range updateQueue {
		go func(i int) {
			pr := updateQueue[i]
			sd.github.UpdatePullRequest(ctx, sd.gitcmd, sortedPullRequests, pr.pr, pr.commit, pr.prevCommit, githubInfo)
			wg.Done()
		}(i)
	}
...
func (sd *stackediff) MergePullRequests(ctx context.Context, count *uint) {
	...
	sd.github.UpdatePullRequest(ctx, sd.gitcmd, githubInfo.PullRequests, prToMerge, prToMerge.Commit, nil, githubInfo)
